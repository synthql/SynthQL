import { AnyDb, AnyQuery, AnyTable, AugmentedQuery, QueryResult, isSelectionColumn, isSelectionJsonbAgg } from "../types";
...
type AnyQueryResult = undefined | Array<AnyQueryResult> | {
    [key: string]: any;
}

export function hydrate(data: Array<Row>, query: AugmentedQuery): AnyQueryResult {
    const mapped = data.map(row => hydrateRow(row, query));
    return applyCardinality(mapped, query.query.cardinality ?? 'many') ?? undefined
}
...
function hydrateNestedMutating(row: Row, parentQuery: AugmentedQuery, result: AnyObject): void {
