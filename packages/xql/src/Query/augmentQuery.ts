import { AnyDb, AnyQuery, AugmentedColumn, AugmentedQuery, AugmentedTable, RefOp, SelectionColumn, isRefOp } from "../types";
...
export function augmentQuery(query: AnyQuery, opts: AugmentOpts): AugmentedQuery {
    ...
    const augQuery: AugmentedQuery = {
        ...
        children: Object.entries(query.include ?? {}).map(([includeColumn, nestedQuery]) => {
...
function augmentLeftJoin(query: AnyQuery, table: AugmentedTable, opts: AugmentOpts): AugmentedQuery['leftJoin'] | undefined {
    if (!opts.depth) {
        return undefined;
    }
...
function augmentTable(tableName: string, defaultSchema: string): AugmentedTable {
...
function createSelectionColumns(query: AnyQuery, table: AugmentedTable): SelectionColumn[] {
    return Object.entries(query.select).map(([column, shouldSelect]): SelectionColumn | undefined => {
        switch (shouldSelect) {
            case true:
                return {
                    type: 'column',
...
function agumentSelect(query: AnyQuery, table: AugmentedTable, opts: AugmentOpts): AugmentedQuery['select'] {
    if (!opts.depth) {
...
function agumentWhere(query: AnyQuery, table: AugmentedTable): AugmentedQuery['where'] {
    return Object.entries(query.where).map(([column, value]): AugmentedQuery['where'][number] | undefined => {
        if (isRefOp(value)) {
            return undefined
        }
        ...
        throw new Error(`Unknown expression type: ${JSON.stringify(value)}`)
    }).filter(isPresent)
}