...
export function* iterateQuery<DB, TTable extends Table<DB>, TQuery extends Query<DB, TTable>>(query: TQuery): Generator<Query<DB, Table<DB>>> {
    yield query;
...
export function* iterateAugmentedQuery(query: AugmentedQuery): Generator<AugmentedQuery> {
    yield query;
