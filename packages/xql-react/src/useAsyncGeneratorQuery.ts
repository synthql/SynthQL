...
export function useAyncGeneratorQuery<TData>({ queryKey = [], queryFn, ...opts }: QueryOptions<AsyncGenerator<TData>>) {
    const { setQueryData } = useQueryClient()

    const streamingQueryResult = useQuery({
        queryKey: [...queryKey, 'streaming-query-result'],
        queryFn: async (): Promise<TData> => {
            return new Promise<TData>(() => { /* suspend forever */ })
        },
        staleTime: Infinity,
    })
    ...
    useQuery({
        ...
        queryFn: async (queryProps): Promise<AsyncGenerator<TData>> => {
            ...
            for await (const result of generator) {
                setQueryData([...queryKey, 'streaming-query-result'], result)
            }
            return generator
        },
        ...opts,
