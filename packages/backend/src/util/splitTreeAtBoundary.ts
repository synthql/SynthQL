...
export function splitTreeAtBoundary<T extends {
    ...
    const queue: { node: T, index: number, parent: T }[] = clone.children.map((node, index) => ({ node, index, parent: clone }));
    ...
    while (queue.length > 0) {
        ...
        if (shouldSplit(head)) {
            ...
            queue.push(...head.children.map((node, index) => ({ node, index, parent })));
        }
    }
    return {
        tree: clone,
