...
export function splitQueryAtBoundary<TQuery extends AnyQuery>(q: TQuery, shouldSplit: (q: TQuery, opts: { depth: number }) => boolean): { query: TQuery, remaining: TQuery[] } {
    const clone = structuredClone(q);
    const remaining: TQuery[] = [];
    for (const { query, parentQuery, includeKey, depth } of iterateQuery(clone)) {
