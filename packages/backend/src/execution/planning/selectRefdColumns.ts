 ...
 */
export function selectRefdColumns<TQuery extends AnyQuery>(query: TQuery, allColumns: ColumnRef[], defaultSchema: string): TQuery {
    const table = TableRef.fromQuery(defaultSchema, query);

    // Filter to only columns from the same table as the query.
    const refdColumns: ColumnRef[] = allColumns.filter(col => col.tableRef.equals(table));
