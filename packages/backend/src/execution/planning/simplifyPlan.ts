 ...
 */
export async function simplifyPlan(plan: ExecPlanTree): Promise<SimplifiedPlan> {
    const tree = await collectLast(
        mapTree<ExecutionPlanNode, SimplifiedPlan>(plan, async (node): Promise<SimplifiedPlan> => {
