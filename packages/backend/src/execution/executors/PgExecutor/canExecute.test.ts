import { filmActor, actor, inventory, film, store } from "../../../tests/queries.v2";
...
describe('PgExecutor', () => {
    ...
    test('canExecute', () => {
        ...
        const inventories = inventory()
            .include({
                film: film().include({ actors }).where({ film_id: col('public.inventory.film_id') }).many()
            })
        ...
        expect(executor.canExecute(q)).toEqual({
            query: { ...q, include: { inventories: { ...inventories, include: {} } } },
