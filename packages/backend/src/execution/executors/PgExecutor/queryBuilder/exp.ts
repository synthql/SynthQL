...
function assertPrimitive(x: unknown, msg?: string): asserts x is Primitive {
    if (!isPrimitive(x)) {
        throw new Error(msg ?? `Expected ${JSON.stringify(x)} to be a primitive but was ${typeof x}`)
    }
}

function assertArray(x: unknown, msg?: string): asserts x is Primitive[] {
    if (!Array.isArray(x)) {
        throw new Error(msg ?? `Expected ${JSON.stringify(x)} to be an array but was ${typeof x}`)
    }
}
...
export class SqlBuilder {
    ...
    addConst(value: Primitive) {
        ...
        if (Array.isArray(value)) {
            return this.openParen()
                .addInterleaved(value.map(v => compileExp(['const', v])), SqlBuilder.comma())
                .closeParen()
        }
    ...
    addSelectColumn(column: ColumnRef) {
        return this
            .add(column.aliasQuoted())
            .space()
            .add('as')
            .space()
            .add('"').add(column.column).add('"')
            .space()
    }
    ...
    addFn(fn: Fn) {
        ...
        return this.add(name)
            .openParen()
            .addInterleaved(args.map(arg => compileExp(arg)), SqlBuilder.comma())
            .closeParen()
    }
    ...
    select(select: SqlBuilder[]) {
        return this.add('select ').addInterleaved(select, SqlBuilder.comma()).space()
    }

    from(table: TableRef) {
        return this.add('from ').add(table.canonical()).space().add(table.aliasQuoted()).space()
    }
    ...
    groupBy(columns: ColumnRef[]) {
        ...
        return this.add('group by ').addInterleaved(aliasedColumns, SqlBuilder.comma()).space()
    }
    ...
    leftJoin(join: Join) {
        ...
        return this.add('left join ')
            .add(table.canonical())
            .space()
            .add(table.aliasQuoted()).add(' on ')
            .addInterleaved(join.conditions.map(cond => {
                const own = cond.ownColumn.aliasQuoted()
                const other = cond.otherColumn.aliasQuoted()
                const op = cond.op
                return new SqlBuilder().add(own).space().add(op).space().add(other).space()
            }), SqlBuilder.and())
    }
    ...
    expressionFromWhere({ table, where = {} }: { table: TableRef, where?: Where<AnyDb, any> }) {
        const expressions = Object.entries(where).map(([column, op]): Exp | undefined => {
            ...
            if (typeof op === 'object' && Object.keys(op).length === 1) {
                ...
                assertOp(opName)
                assertPrimitive(value, `Expected value ${JSON.stringify(value)} to be a primitive in ${JSON.stringify(op)}`)
    ...
    expressionFromManyWhere(where: { table: TableRef, where?: Where<AnyDb, any> }[]) {
        const expressions = where.map(w => new SqlBuilder().expressionFromWhere(w)).filter(b => !b.isEmpty())
        if (expressions.length === 0) {
            return this
        }
