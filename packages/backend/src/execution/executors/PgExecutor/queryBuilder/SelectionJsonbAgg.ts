...
export class SelectionJsonbAgg implements Selection {
    ...
    static fromQuery(rootQuery: AnyQuery, defaultSchema: string): SelectionJsonbAgg[] {
        if (rootQuery.include === undefined) {
            return []
        }
        const result: SelectionJsonbAgg[] = []
        for (const [selectionId, subQuery] of Object.entries(rootQuery.include)) {
            result.push(new SelectionJsonbAgg(
                selectionId,
