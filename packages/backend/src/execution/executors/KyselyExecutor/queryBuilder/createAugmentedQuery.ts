...
export function createAugmentedQuery(rootQuery: AnyQuery, defaultSchema: string): AugmentedQuery {
    const rootTable = TableRef.fromQuery(defaultSchema, rootQuery)
    const selectionColumn: Selection[] = SelectionColumn.fromQuery(rootQuery, defaultSchema);
    const jsonbAggColumn: Selection[] = SelectionJsonbAgg.fromQuery(rootQuery, defaultSchema);
    const wheres = collectWhere(rootQuery, defaultSchema);
    const joins = collectJoins(rootQuery, defaultSchema);
    const groupingColumns = rootQuery.groupingId?.map(col => rootTable.column(col)) ?? []
...
function collectJoins(query: AnyQuery, defaultSchema: string): Array<Join> {
    return collectFromQuery(query, query => {
        const conditions = Object.entries(query.where).flatMap(([column, refOp]) => {
            if (!isRefOp(refOp)) {
                return []
            }
            return [
                {
                    ownColumn: TableRef.fromQuery(defaultSchema, query).column(column),
...
function collectWhere(query: AnyQuery, defaultSchema: string): Array<{ table: TableRef, where: Where<AnyDb, string> }> {
    return collectFromQuery(query, query => {
        return [{ table: TableRef.fromQuery(defaultSchema, query), where: query.where }]
    })
}