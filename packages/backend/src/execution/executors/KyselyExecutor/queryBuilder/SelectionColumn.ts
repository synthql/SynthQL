...
export class SelectionColumn implements Selection {
    ...
    public constructor(
        public readonly column: ColumnRef,
    ) {
    }

    static fromQuery(rootQuery: AnyQuery, defaultSchema: string): SelectionColumn[] {
    ...
    extractFromRow(row: any, target: any): void {
        ...
        if (tmp === undefined) {
            throw new Error(`Column ${this.column.column} not found in row: ${JSON.stringify(row)}`)
        }
