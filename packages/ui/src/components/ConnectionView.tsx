...
import { Box, Button, Card, CardActionArea, CardContent, CircularProgress, FormControl, InputLabel, TextField, Typography, useTheme } from "@mui/material";
...
export function ConnectionView() {
    ...
    return <Box height={'100%'} display={'flex'} flexDirection={'column'} alignItems={'center'} justifyContent={'center'} sx={{
        background
    }}>
        <Card>
            <CardContent sx={{
                ...
                <TextField
                    ...
                    label="Name" helperText="A user friendly name for your connection" />

                <Button
                    startIcon={isPending ? <CircularProgress size={'12px'} /> : undefined}
                    disabled={isPending}
                    onClick={() => onConnect({
                        schema,
                        url
